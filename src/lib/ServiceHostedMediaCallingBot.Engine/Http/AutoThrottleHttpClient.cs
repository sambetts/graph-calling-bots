...
public class AutoThrottleHttpClient : HttpClient
{
    ...
    public async Task<HttpResponseMessage> ExecuteHttpCallWithThrottleRetries(Func<Task<HttpResponseMessage>> httpAction, string url)
    {
        ...
        while (retryDownload)
        {
            ...
            if (!response.IsSuccessStatusCode && (int)response.StatusCode == 429)
            {
                ...
                var waitValue = GetRetryAfterHeaderSeconds(response);
                if (!ignoreRetryHeader && waitValue.HasValue)
                {
                    secondsToWait = waitValue.Value;
                    _debugTracer.LogTrace($"{THROTTLE_ERROR} for {url}. Waiting to retry for attempt #{retries}, {secondsToWait} seconds (from 'retry-after' header)...");
                }
                else
                {
                    ...
                    _debugTracer.LogTrace($"{THROTTLE_ERROR} downloading from REST. Waiting {retries} seconds to try again...");
    ...
    public static int? GetRetryAfterHeaderSeconds(HttpResponseMessage response)
    {
